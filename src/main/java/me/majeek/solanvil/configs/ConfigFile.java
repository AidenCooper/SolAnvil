package me.majeek.solanvil.configs;

import com.google.common.collect.Lists;
import me.majeek.solanvil.SolAnvil;
import me.majeek.solanvil.configs.resolver.Resolver;
...
public class ConfigFile {
    ...
    public ConfigFile(String fileName) {
        ...
        this.file = new File(SolAnvil.getInstance().getDataFolder(), fileName + ".yml");
    ...
    public ConfigFile(String path, String fileName) {
        this.fileName = fileName.replaceAll(".yml", "");
        this.file = new File(new File(SolAnvil.getInstance().getDataFolder(), path), fileName + ".yml");
        this.configuration = YamlConfiguration.loadConfiguration(file);
        this.annotatedClasses = Lists.newArrayList();
    }
    ...
    public ConfigFile createFile(boolean saveResource) {
        if (!file.exists()) {
            try {
                ...
                if (saveResource) {
                    SolAnvil.getInstance().saveResource(file.getName(), true);
                } else {
                    file.createNewFile();
                }
            } catch (IOException exception) {
                SolAnvil.getInstance().getLogger().log(Level.SEVERE, "Could not create " + this.file.getPath(), exception);
            }
        }
    ...
    public ConfigFile loadConfig() {
        try {
            ...
            SolAnvil.getInstance().getLogger().log(Level.SEVERE, "Could not load " + this.file.getPath(), exception);
        }
    ...
    public ConfigFile saveConfig() {
        try {
            ...
            SolAnvil.getInstance().getLogger().log(Level.SEVERE, "Could not save " + this.file.getPath(), exception);
        }
        ...
        annotatedClasses.forEach(clazz -> {
            ...
            fields.forEach(field -> {
                ...
                try {
                    ...
                    SolAnvil.getInstance().getLogger().log(Level.SEVERE, "Could not export " + this.file.getPath(), exception);
                }
            });
        });
        ...
        annotatedClasses.forEach(clazz -> {
            ...
            fields.forEach(field -> {
                ...
                try {
                    ...
                    SolAnvil.getInstance().getLogger().log(Level.SEVERE, "Could not set annotations at " + this.file.getPath(), exception);
                }
            });
        });
    ...
    public ConfigFile setAnnotatedField(Field field, boolean overwrite) {
        ...
        try {
            ...
            SolAnvil.getInstance().getLogger().log(Level.SEVERE, "Could not set annotation field at " + this.file.getPath(), exception);
        }
